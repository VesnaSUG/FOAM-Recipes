...
```
$ node foam3/tools/build.js -Ttemplate/demo/Project --appName:Recipe --package:com.foamdev.cook.recipe
...
The key entity for a recipe database is Recipe.  Open file src/foam/foamdev/cook/recipe/Recipe.js:

```
foam.CLASS({
  package: 'com.foamdev.cook.recipe',
  name: 'Recipe',
  ...
  properties: [
    ...
    {
      class: 'Enum',
      of: 'com.foamdev.cood.recipe.RecipeCategory',
      name: 'category',
      value: 'OTHER'
    },
    {
...
```
Here we defined a model for class <code>Recipe</code> that will be in the <code>com.foamdev.cook.recipe</code> package with several properties and a few sample methods.
...
VESNA: this files of the original pom are now in src/com/foamdev/cook/recipe/pom.js
...
p({
  ...
  "serviceScript": """
    return new foam.dao.EasyDAO.Builder(x)
      .setOf(com.foamdev.cook.recipe.Recipe.getOwnClassInfo())
      .setPm(true)
      .setFuid(true)
      .setAuthorize(false)
      ...
      .build();
  """,
  "client": `{"of":"com.foamdev.cook.recipe.Recipe"}`
})
```
...
# Testing
As you develop, also consider creating tests cases to exercise your models and application. An example is provided in <code>src/com/foamdev/cook/recipe/test/RecipeTest.js</code>, with configuration to run it in <code>development/test/test.sh</code>.
